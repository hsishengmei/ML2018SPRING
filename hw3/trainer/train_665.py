...
def train(X, Y, model_path = 'trainer/tmp/model_665.h5'):
    
    X = X[:,:,:,np.newaxis] / 255
    X, valX, Y, valY = train_test_split(X, Y, test_size=0.1, random_state=8)
    ...
    model.add(act2)
    model.add(Dropout(0.5))
    act3 = advanced_activations.PReLU(init='zero', weights=None)
    model.add(Dense(64))
    model.add(act3)
    model.add(Dropout(0.5))
...
if __name__ == '__main__':
    ...
    train(X, Y, model_path = 'tmp/model_665.h5')